...
Full-page capture is the default for every route and viewport. `/snapshot` reuses the `/screenote` capture procedure, so each capture scrolls lazy-loaded content first, keeps sticky elements in place, and caps tall pages at **5000 px / 10 scrolls**.

...
Full-page capture is the default for every route and viewport. `/snapshot` reuses the `/screenote` capture procedure, so each capture scrolls lazy-loaded content first, keeps sticky elements in place, and caps tall pages at **5000 px or 10 scrolls, whichever fires first**.
...
2. **Run the `/screenote` Step 4 capture-and-upload procedure** against the returned `uploads` array. Before each viewport capture, set `SCREENOTE_OUTPUT="$SCREENOTE_DIR/<i>-<viewport>.png"` so the shared procedure writes route-scoped files and appends route/viewport status to `run-status.jsonl`.
